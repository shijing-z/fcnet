...
to compute an upper triangle. Use [`calc_conn`](calc_conn.md) for
single-ROI connectivity.

## See also

[`calc_between`](calc_between.md) for between-network connectivity.
[`calc_conn`](calc_conn.md) for user-defined ROI-to-ROI or
ROI-to-network connectivity. [`get_indices`](get_indices.md) for
generating the `indices` input.
