...
test_that("calc_between errors with invalid indices", {
  # Unnamed list
  expect_error(
    calc_between(example_conn_array, list(1:5, 6:10)),
    "named list"
  )

  # Not a list
  expect_error(
    calc_between(example_conn_array, c(1, 2, 3)),
    "named list"
  )
})
...
test_that("calc_between errors with invalid pairwise parameter", {
  ...
  expect_error(
    calc_between(example_conn_array, indices, pairwise = "yes"),
    "TRUE or FALSE"
  )

  expect_error(
    calc_between(example_conn_array, indices, pairwise = c(TRUE, FALSE)),
    "TRUE or FALSE"
  )
})
...
test_that("calc_between warns and drops entries with fewer than 2 ROIs", {
  ...
  expect_warning(
    result <- calc_between(example_conn_array, indices),
...
test_that("calc_between warning suggests calc_conn", {
  ...
  expect_warning(
    calc_between(example_conn_array, indices),
    "calc_conn"
  )
})
...
test_that("calc_between errors when fewer than 2 networks remain", {
  ...
  expect_error(
    calc_between(example_conn_array, one_net_indices),
    "At least 2 networks"
  )
})
...
test_that("calc_between per-network computes correctly", {
  ...
  expected_vis <- mean(example_conn_array[vis_idx, other_idx, 1])
...
test_that("calc_between pairwise mode returns correct structure", {
  indices <- get_indices(example_conn_array, roi_include = "schaefer")
  result <- calc_between(example_conn_array, indices, pairwise = TRUE)
...
test_that("calc_between pairwise computes correctly", {
  ...
  expected_pair <- mean(example_conn_array[def_idx, cont_idx, 1])
...
test_that("calc_between between_network uses raw cell average", {
  ...
  for (net in network_names) {
    ...
    net_between <- example_conn_array[target_idx, other_idx, 1]
...
test_that("calc_between works with two networks", {
  ...
  # Per-network mode
  result <- calc_between(example_conn_array, two_net_indices)
  expect_equal(ncol(result), 3) # between_vis, between_default, between_network

  # Pairwise mode: 2 networks → 1 pair
  result_pw <- calc_between(example_conn_array, two_net_indices, pairwise = TRUE)
